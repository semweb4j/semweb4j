...
public class TypeConversion {
	 ...
	 * the logger
	 */
	static Log log = LogFactory.getLog(TypeConversion.class);
	 ...
	 */
	public static Node toJenaNode(Object o,
			com.hp.hpl.jena.rdf.model.Model model) throws RuntimeException {
		assert o != null;
		if (o instanceof URI) {
		...
		if (o instanceof PlainLiteral) {
			// plain literal
			log.debug("instanceof String");
			return Node.createLiteral(((PlainLiteral) o).getValue(), null,
					false);
		}
		...
		if (o instanceof DatatypeLiteral) {
			...
			Literal l = model
					.createTypedLiteral(((DatatypeLiteral) o).getValue(),
	 ...
	 *             if the object can't be transformed in a jena node
	 */
	public static Node toJenaNode(Object o) throws RuntimeException {
		...
		if (o instanceof DatatypeLiteral) {
			...
			return Node.createLiteral(((DatatypeLiteral) o).getValue(), null,
					new GeneralDataType(((DatatypeLiteral) o).getDatatype()
							+ ""));
		}
	...
	public static org.ontoware.rdf2go.model.node.Node toRDF2Go(Node n)
		...
		if (n.isLiteral()) {
			...
			if (lit.getDatatypeURI() != null) {
				return new DatatypeLiteralImpl(lit.getLexicalForm(),
						new URIImpl(lit.getDatatypeURI()));
			}

			// language tagged
			if (lit.language() != null && !lit.language().equals(""))
				return new LanguageTagLiteralImpl(lit.getLexicalForm(), lit
						.language());
