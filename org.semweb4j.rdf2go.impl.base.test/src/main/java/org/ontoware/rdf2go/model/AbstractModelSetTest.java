...
public abstract class AbstractModelSetTest {
	...
	public void testCopyModelSets() throws Exception {
		...
		assertEquals("copied all from source to target", this.modelset.size(), modelset2.size());
		modelset2.close();
	}
	...
	public void testDeleteStatement() {
		...
		this.modelset.addStatement(null, a, b, c);
		Assert.assertEquals(2, this.modelset.size());
		this.modelset.removeStatement(this.modelset.createStatement(null, a, b, c));
		Assert.assertEquals(1, this.modelset.size());
	}
	...
	public void testFindStatements() throws ModelRuntimeException {
		...
		ClosableIterator<? extends Statement> it = this.modelset.findStatements(graphuri1, a, b, c);
	...
	public void testGetDefaultModel() {
		...
		assertTrue("new open/close policy: open modelsets return open models", defaultModel
		        .isOpen());
	...
	public void testGetModel() throws Exception {
		...
		m.open();
		assertNotNull(m);
		assertTrue("graph1 contains less than 10 statements, it contains: " + m.size(),
		        m.size() > 10);
		m = this.modelset.getModel(graphuri2);
		m.open();
		assertNotNull(m);
		assertTrue("graph2 contains more than 10 statements, it contains: " + m.size(),
		        m.size() > 10);
		m.close();
	}
	...
	public void testRDF2GoBug() {
		for(ClosableIterator<? extends Statement> i = this.modelset.iterator(); i.hasNext();) {
			i.next();
		}
	}
	...
	public void testReadFromInputStreamIntoNamedModel() throws Exception {
		...
		int sizeByIterator = TestUtils.countAndClose(m);
		assertEquals("the model supports iterators", TestData.FOAFSIZE, sizeByIterator);
		m.close();
	...
	public void testRTGO_39() {
		for(ClosableIterator<Statement> i = this.modelset.iterator(); i.hasNext();) {
			i.next();
		}
	}
	...
	public void testSparqlSelect() throws ModelRuntimeException {
		...
		QueryResultTable table = this.modelset.sparqlSelect("SELECT ?s ?p ?o WHERE { ?s ?p ?o . }");
	...
	public void testCountStatements() {
		...
		assertEquals(0, this.modelset.countStatements(this.modelset.createQuadPattern(a, subject,
		        predicate, object)));
		this.modelset.addStatement(statement);
		assertEquals(1, this.modelset.countStatements(this.modelset.createQuadPattern(a, subject,
		        predicate, object)));
	}
	...
	public void testDeleteWithBlankNodes() {
		...
		assertFalse(this.modelset.containsStatements(a, blankNode, RDF.type, RDF.Statement));
		Statement stmt = this.modelset.createStatement(a, blankNode, RDF.type, RDF.Statement);
		assertFalse(this.modelset.containsStatements(a, blankNode, RDF.type, RDF.Statement));
	...
	public void testDeleteReification() {
		assertFalse(this.modelset.containsStatements(a, Variable.ANY, RDF.subject, subject));
	...
	public void testRemoveStatement() {
		...
		assertTrue(this.modelset.contains(this.modelset.createStatement(a, subject, predicate,
		        object)));
		this.modelset.removeStatement(a, subject, predicate, object);
		assertFalse(this.modelset.contains(this.modelset.createStatement(a, subject, predicate,
		        object)));
	}
	...
	public void testRemoveStatements() {
		...
		assertTrue(this.modelset.contains(this.modelset.createStatement(a, subject, predicate,
		        object)));
		this.modelset.removeStatements(this.modelset.createQuadPattern(a, subject, predicate,
		        object));
		assertFalse(this.modelset.contains(this.modelset.createStatement(a, subject, predicate,
		        object)));
