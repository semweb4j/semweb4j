...
import org.ontoware.rdf2go.impl.jena24.DumpUtils;
...
public class FoafExample {
	...
	public static void main(String[] args) throws Exception {
		...
		// no inferencing
		Model model = new ModelImplJena24(Reasoning.none);
		// alternatives
		// new ModelImplYars();
		// new ModelImplNG4J();
		...
		ClosableIterator<? extends Statement> it = model.findStatements(Variable.ANY, RDF.type, foafPerson);
		while (it.hasNext()) {
			Resource person = it.next().getSubject();
			System.out.println(person + " is a person");

			// get foaf:name
			ClosableIterator<? extends Statement> it2 = model.findStatements(person, foafName, Variable.ANY);
			while (it2.hasNext()) {
