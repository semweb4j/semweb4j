...
public class CodeGenerator {
	...
	public static void generate(String schemafilename, String outdir, String packagename,
		...
		generate(schemafilename, outdir, packagename, semantics, skipbuiltins, alwaysWriteToModel,
				"");
	}
	...
	public static void generate(String schemafilename, String outdir, String packagename,
		...
		generate(schemaDataModel, outDir, packagename, semantics, skipbuiltins, alwaysWriteToModel,
				methodnamePrefix);
	}
	...
	public static void generate(String schemafilename, String outdir, String packagename,
		...
		generate(schemafilename, outdir, packagename, reasoning, skipbuiltins, alwaysWriteToModel,
				methodnamePrefix);
	}
	 ...
	 * @throws Exception
	 */
	public static void generate(com.hp.hpl.jena.rdf.model.Model schemaDataModel, File outDir,
			String packagename, Reasoning semantics, boolean skipbuiltins,
			boolean alwaysWriteToModel, String methodnamePrefix) throws Exception {
		...
		if (semantics == Reasoning.rdfs) {
			...
			jm = ModelGenerator.createFromRDFS(schemaDataModel, packagename, skipbuiltins);
		} else if (semantics == Reasoning.owl) {
			log.info("generating OWL into " + outDir.getAbsolutePath() + " ...");
			jm = ModelGenerator.createFromOWL(schemaDataModel, packagename, skipbuiltins);
		} else if (semantics == Reasoning.rdfsAndOwl) {
			log.info("MODEL generating RDFS+OWL mix semantics into " + outDir.getAbsolutePath()
					+ " ...");
			jm = ModelGenerator.createFromRDFS_AND_OWL(schemaDataModel, packagename, skipbuiltins);
		} else
	 ...
	 * @throws Exception
	 */
	public static com.hp.hpl.jena.rdf.model.Model loadSchemaDataModel(String schemafilename)
			throws Exception {
		File schemaFile = new File(schemafilename);
		log.info("loading from " + schemaFile.getCanonicalPath());
		com.hp.hpl.jena.rdf.model.Model schemaDataModel = ModelReaderUtils.read(schemafilename);
