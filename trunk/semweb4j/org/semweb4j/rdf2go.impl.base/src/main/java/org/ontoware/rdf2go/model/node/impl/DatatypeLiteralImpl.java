...
public class DatatypeLiteralImpl extends LiteralImpl implements DatatypeLiteral {
	...
	// IMPROVE: (from wth) perhaps add xsd-Uri to primitive type conversion
	// directly
	// 
	/*
	 * (non-Javadoc)
	 * 
	 * @see org.ontoware.rdf2go.DatatypeLiteral#getValueAsPrimitiveJavaType(java.lang.Class)
	 */
	public Object getValueAsPrimitiveJavaType(Class primitiveJavaType) {
		// Integer
		if (primitiveJavaType.equals(Integer.class))
			return Integer.parseInt(this.value);

		// Long
		if (primitiveJavaType.equals(Long.class))
			return Long.parseLong(this.value);

		// rdf2go URI
		// TODO return as java.net.URI

		// Double
		if (primitiveJavaType.equals(Double.class))
			return Double.parseDouble(this.value);

		// Float
		if (primitiveJavaType.equals(Float.class))
			return Float.parseFloat(this.value);

		// Boolean
		if (primitiveJavaType.equals(Boolean.class))
			return Boolean.parseBoolean(this.value);

		// Byte
		if (primitiveJavaType.equals(Byte.class))
			return Byte.parseByte(this.value);

		// Char
		if (primitiveJavaType.equals(Character.class)) {
		...
		// unknown type
		throw new IllegalArgumentException(primitiveJavaType
				+ " is not a known primitive type in java");
	}
	...
	public boolean equals(Object o) {
		return ((o instanceof DatatypeLiteralImpl)
				&& (this.getValue()
						.equals(((DatatypeLiteralImpl) o).getValue())) && (this
	...
	public int asInt() throws ClassCastException, NumberFormatException {
		if (getDatatype().equals(XSD._int)
				|| getDatatype().equals(XSD._nonPositiveInteger)
				|| getDatatype().equals(XSD._long)
				|| getDatatype().equals(XSD._nonNegativeInteger)
				|| getDatatype().equals(XSD._negativeInteger)
				|| getDatatype().equals(XSD._integer)
				|| getDatatype().equals(XSD._short)
				|| getDatatype().equals(XSD._byte)
