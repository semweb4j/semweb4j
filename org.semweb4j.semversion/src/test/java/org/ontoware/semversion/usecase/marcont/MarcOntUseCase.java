...
public class MarcOntUseCase
{
	...
	public void testMarcOntUseCase() throws Exception
	{
		...
		User user_d = semVersion.createUser("User d", "secret");
		user_d.getURI();
		...
		// model.addStatement(Object subject , URI predicate , String literal, String languageTag),
		// or model.addStatement(Object subject, URI predicate, String literal, URI dataTypeURI).
		...
		vm2.getLastMainbranchVersion().commit(suggestion2, "Suggestion of user b", true);
		...
		// commit the suggestion
		vm3.getLastMainbranchVersion().commit(suggestion3, "Suggestion of user c", true);
		...
		// commit the suggestion
		vm4.getLastMainbranchVersion().commit(suggestion4, "Suggestion of user d", true);
		...
		List<Version> allVersions = vm5.getLastMainbranchVersion().getSuggestions();
		...
		//-----------------------------------------------------------------------------------
		// User B commits another Suggestion as a diff to the most recent version
		//-----------------------------------------------------------------------------------
		...
		// commit the diff
		vm6.getLastMainbranchVersion().commit(diff, "Another suggestion", new URIImpl("data://suggestion6"), null, true);
		
		//-----------------------------------------------------------------------------------
		// User A wants to see the differences between the first version and the latest version
		//-----------------------------------------------------------------------------------
		...
		// calculate the semantic diff of the root Version and the latest main-branch version
		Diff diff2 = SemanticDiffEngine.getSemanticDiff_RDFS(vm7.getRoot().getContent(), vm7.getLastMainbranchVersion().getContent());
